...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function inicializarCurso() {
        ...
        const perguntas = [
            { pergunta: "Qual é a função primária de uma bomba em um sistema óleo-hidráulico?", opcoes: ["Gerar pressão", "Gerar vazão (fluxo)", "Controlar a temperatura", "Filtrar o fluido"], resposta: "Gerar vazão (fluxo)" },
            { pergunta: "O Princípio de Pascal é a base para qual fenômeno hidráulico?", opcoes: ["A conversão de velocidade em pressão", "A multiplicação de força", "O escoamento laminar", "A perda de carga"], resposta: "A multiplicação de força" },
            { pergunta: "Um Número de Reynolds (Re) significativamente acima de 3.000 em uma tubulação indica que o escoamento é:", opcoes: ["Laminar", "Estático", "De transição", "Turbulento"], resposta: "Turbulento" },
            { pergunta: "De acordo com o Princípio de Bernoulli, o que acontece com a pressão estática de um fluido quando sua velocidade aumenta significativamente em uma restrição?", opcoes: ["Aumenta", "Diminui", "Permanece a mesma", "Torna-se negativa"], resposta: "Diminui" },
            { pergunta: "Qual componente é projetado especificamente para converter energia hidráulica em movimento mecânico linear?", opcoes: ["Motor hidráulico", "Bomba de engrenagens", "Cilindro hidráulico", "Válvula de alívio"], resposta: "Cilindro hidráulico" },
            { pergunta: "A perda de carga em um circuito hidráulico está diretamente associada a qual fenômeno indesejado?", opcoes: ["Aumento da força do atuador", "Redução do volume do reservatório", "Geração de calor e perda de eficiência", "Aumento da viscosidade do óleo"], resposta: "Geração de calor e perda de eficiência" },
            { pergunta: "Qual é a principal vantagem dos sistemas hidráulicos em comparação com os mecânicos e elétricos?", opcoes: ["Baixo custo inicial", "Alta eficiência energética", "Alta densidade de potência (relação força/tamanho)", "Não necessitam de manutenção"], resposta: "Alta densidade de potência (relação força/tamanho)" },
            { pergunta: "A contaminação por partículas sólidas em um sistema hidráulico afeta principalmente qual aspecto?", opcoes: ["A cor do óleo", "O peso total do sistema", "A vida útil dos componentes e a confiabilidade", "A pressão atmosférica"], resposta: "A vida útil dos componentes e a confiabilidade" },
            { pergunta: "Qual subsistema é responsável por regular a direção, pressão e vazão do fluido?", opcoes: ["Unidade de Geração de Potência (HPU)", "Subsistema de Atuação", "Circuito de Comando e Controle", "Reservatório Hidráulico"], resposta: "Circuito de Comando e Controle" },
            { pergunta: "O que significa o termo 'comprimento equivalente' (Ls) no cálculo de perda de carga?", opcoes: ["A distância total da tubulação", "Um comprimento de tubo reto que causaria a mesma perda de uma singularidade (curva, válvula, etc.)", "A altura da coluna de fluido", "O diâmetro da tubulação"], resposta: "Um comprimento de tubo reto que causaria a mesma perda de uma singularidade (curva, válvula, etc.)" }
        ];
        let perguntaAtual = 0, pontuacao = 0;
        const perguntaTituloEl = document.getElementById('pergunta-titulo'), opcoesQuizEl = document.getElementById('opcoes-quiz'), feedbackEl = document.getElementById('feedback'), quizContainerEl = document.getElementById('quiz-container'), aprovadoEl = document.getElementById('aprovado-container'), reprovadoEl = document.getElementById('reprovado-container');
        
        function iniciarQuiz() {
            perguntaAtual = 0; pontuacao = 0;
            if(feedbackEl) feedbackEl.textContent = '';
            if(aprovadoEl) aprovadoEl.style.display = 'none';
        ...
        function finalizarQuiz() {
            ...
            if ((pontuacao / perguntas.length) >= 0.7) {
                if(aprovadoEl) aprovadoEl.style.display = 'block';
