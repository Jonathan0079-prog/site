...
function inicializarCurso() {
    ...
    const perguntaTituloEl = document.getElementById('pergunta-titulo'),
        opcoesQuizEl = document.getElementById('opcoes-quiz'),
        feedbackEl = document.getElementById('feedback'),
        quizContainerEl = document.getElementById('quiz-container'),
        certificadoFormEl = document.getElementById('certificado-form-container'),
        reprovadoEl = document.getElementById('reprovado-container');
    ...
    function mostrarPergunta() {
        ...
        p.opcoes.forEach(op => { 
            ...
            btn.textContent = op; 
            btn.onclick = () => verificarResposta(op, p.resposta); 
    ...
    function finalizarQuiz() {
        ...
        if ((pontuacao / perguntas.length) >= 0.6) { // 60% para passar
            ...
            // Avança automaticamente para o módulo do certificado
    ...
    function gerarCertificadoPDF() {
        const nome = document.getElementById('nome-aluno').value.trim(),
            documento = document.getElementById('documento-aluno').value.trim(),
            pais = document.getElementById('pais-aluno').value;
        ...
        doc.text("CERTIFICADO DE CONCLUSÃO", 148.5, 30, { align: "center" });

        // Nome e texto principal
        doc.setFont("helvetica", "normal");
        doc.setFontSize(16);
        doc.setTextColor(50, 50, 50);
        doc.text(`Certificamos que`, 148.5, 45, { align: "center" });
        ...
        doc.text(`${docTexto}, concluiu com aproveitamento o curso de`, 148.5, 65, { align: "center" });

        doc.setFont("helvetica", "bold");
        doc.setFontSize(18);
        doc.setTextColor(0, 51, 102);
        doc.text("HIDRÁULICA INDUSTRIAL", 148.5, 75, { align: "center" });

        // Carga horária
        doc.setFont("helvetica", "normal");
        doc.setFontSize(14);
        doc.setTextColor(50, 50, 50);
        doc.text("Carga horária: 40 horas", 148.5, 85, { align: "center" });

        // Conteúdo programático em duas colunas
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.text("Conteúdo Programático:", 20, 95);

        doc.setFont("helvetica", "normal");
        doc.setFontSize(12);
        const conteudoEsquerda = [
            "• Aula 1: Introdução",
            "• Aula 2: Fundamentos",
            "• Aula 3: Simbologia",
            "• Aula 4: Fluidos",
        ...
        let y = 102;
        conteudoEsquerda.forEach(item => {
            doc.text(item, 20, y);
            y += 6;
        });
        y = 102;
        conteudoDireita.forEach(item => {
            doc.text(item, 160, y); // 160mm para a coluna da direita
            y += 6;
        });
    ...
    const paisSelect = document.getElementById('pais-aluno'),
        docLabel = document.getElementById('documento-label'),
        docInput = document.getElementById('documento-aluno');

    if (paisSelect && docLabel && docInput) {
        paisSelect.addEventListener('change', function() {
            docLabel.textContent = this.value === 'angola' ? 'Seu BI:' : 'Seu CPF:';
            docInput.placeholder = this.value === 'angola' ? 'Digite seu BI' : 'Digite seu CPF';
            docInput.value = '';
        });

        docInput.addEventListener('input', function() {
            if (paisSelect.value === 'brasil') this.value = formatarCPF(this.value);
        });
