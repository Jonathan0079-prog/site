# app.py - Versão FINAL e OTIMIZADA para Chat de Texto (Llama 3 via Hugging Face Direto)
...
def get_text_client():
    """Cria e retorna um cliente para o modelo de linguagem de texto (Llama 3 Instruct via HF Direto)."""
    if not HUGGING_FACE_TOKEN:
        raise ValueError("Token da Hugging Face (HF_TOKEN) não encontrado.")
    # MODELO APONTA DIRETAMENTE PARA O Llama 3 8B Instruct na Hugging Face
...
@app.route('/')
def index():
    return "Servidor da AEMI (versão Otimizada para Chat de Texto com Llama 3) está no ar."

@app.route('/chat', methods=['POST'])
def chat():
    try:
        user_message = request.form.get('message', '')
        # A leitura de 'file' foi removida, pois não será mais usada.
        ...
        # O Llama 3 8B tem uma janela de contexto de 8k tokens. 20 mensagens (10 pares) é um bom ponto de partida.
        if len(current_history) > 20: 
            # Garante que a instrução do sistema (primeira mensagem) seja sempre mantida
            system_prompt = current_history[0]
            recent_history = current_history[-19:] # Pega as últimas 19 mensagens, excluindo a do sistema
            session['conversation_history'] = [system_prompt] + recent_history
        else:
            session['conversation_history'] = current_history # Salva o histórico atualizado na sessão
