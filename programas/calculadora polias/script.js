...
document.addEventListener('DOMContentLoaded', () => {
 ...
 if (dom[id] && state[id] !== undefined && id !== 'diametroMotora' && id !== 'diametroMovida') {
    ...
    function updateDiagramPreview() {
        ...
        const c = parseFloat(dom.distanciaEixos.value);

        if (!d1 || !d2 || !c || d1 <= 0 || d2 <= 0 || c <= 0) {
            resetDiagram();
        } else {
            drawDiagram({ d1, d2, c });
        }
    ...
    function handleProjectListClick(e) {
        ...
        const projects = JSON.parse(localStorage.getItem('pulleyProjects')) || [];
        const projectToHandle = projects[index];

        if (target.closest('.delete-project-btn')) {
            showModal(`Tem a certeza que deseja apagar o projeto "${projectToHandle.name}"?`, 'confirm', () => {
                projects.splice(index, 1);
                localStorage.setItem('pulleyProjects', JSON.stringify(projects));
                loadProjects();
            });
        } else if (target.tagName === 'SPAN') {
            loadSolutionIntoDirectForm(projectToHandle);
        }
    }
    ...
    function loadSolutionIntoDirectForm(sol) {
        ...
        // Popula os selects de diâmetro de forma síncrona
        updatePulleySelects();

        // Agora que as opções foram criadas, podemos definir os valores com segurança
 ...
 if (dom['direct-results-container']) dom['direct-results-container'].style.display = 'block';
        ...
        populateSelect(dom.failureType, diagOpts, (opt) => ({ value: opt, text: DB.diagnosis[opt] }));
